 ...
 * @author gabrieli
 */
public class EditarTurmaSwing extends javax.swing.JFrame {
 
    Turma turma;
    DefaultListModel listaAlunos = new DefaultListModel();
    
    public EditarTurmaSwing(Turma turma) {
        initComponents();
        ...
        jTFNomeTurma.setText(turma.getNome());
        jYCAnoInicialTurma.setYear(turma.getAnoInicio());
        if (turma.getCurso().equals("Informatica")) {
            jCBCursoTurma.setSelectedIndex(1);
        } else {
            jCBCursoTurma.setSelectedIndex(2);
        }
        jTFPontuacaoTurma.setText(String.valueOf(turma.getPontos()));
        
        for (int i = 0; i < turma.getAlunos().size(); i++) {
            listaAlunos.addElement(turma.getAlunos().get(i));
        }
    ...
    private void jBExcluirTurmaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBExcluirTurmaActionPerformed
        int resp = JOptionPane.showConfirmDialog(null, "Deseja excluir essa turma?", "Excluir turma", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (resp == JOptionPane.YES_OPTION) {
            try {
                new RequisicaoHttp().deleteTurma(turma);
            } catch (Exception ex) {
    ...
    private void jBSalvarTurmaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBSalvarTurmaActionPerformed
        //salvar alterações da turma no banco
        if (!jTFNomeTurma.getText().equals("") && jYCAnoInicialTurma.getYear()!=0 && !jTFPontuacaoTurma.getText().equals("") && jCBCursoTurma.getSelectedIndex() != 0) {
            if (jListaAlunosTurma.getModel().getSize()!= 0) {
                ...
                try {
                    ...
                    new RequisicaoHttp().updateTurma(turma);
                } catch (Exception ex) {
                    Logger.getLogger(EditarTurmaSwing.class.getName()).log(Level.SEVERE, null, ex);
                }
